...
import { createOrGetThread } from "@/lib/services/messages";
import type { CreateThreadData } from "@/types/message";
...
export default function Checkout() {
	...
	const handlePayment = async () => {
		...
		try {
			...
			setBookingRef(`TH-${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}-${newReservationId.substring(0, 6).toUpperCase()}`);
	...
	const handleMessageHost = async () => {
		if (!userId || !reservationId || !urlHostId || !urlHostName) {
			alert("Missing required information to start conversation");
			return;
		}

		try {
			// Create or get existing thread for this reservation
			...
			const threadId = await createOrGetThread(threadData);
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					<div className="bg-white rounded-[24px] p-6 border border-[#F3F4F6] shadow-sm">
						...
						<div className="mt-5 flex flex-wrap gap-3">
							<button className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 transition-colors">
